...
char* pop() {
  ...
  if( head != NULL){   // don't pop an empty list!
    val = head->data_str;  // grab the value and next pointer from
...
void appendNode(char* data) {
  ...
  newNode = (struct node*)malloc(sizeof(char)*strlen(data) + 1);
  newNode->data_str = data;
  newNode->next = NULL;
...
struct node* copyList(struct node* starter) {
    ...
    while (current != NULL) {
        ...
        newNode->data_str = current->data_str; /* reassign data */
        newNode->next = NULL;
