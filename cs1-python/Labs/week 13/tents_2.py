...
class TentsConfig():
    ...
    """
    __slots__ = ('DIM', 'colVals', 'rowVals', 'board', 'cursor', 'trees')
...
def isGoal(config):
    ...
    for r in range(config.DIM):
        for c in range(config.DIM):
            if config.board[r][c] == TentsConfig.TENT:
                rows[r] += 1 #increment values in the lists
                cols[c] += 1
    if rows == config.rowVals and cols == config.colVals:
        #if number of tents input from the file and number of placed
        #tents are equal 
        return True #lists equal eachother
...
def isValid(config):
    ...
    if row >= 0 and col >= 0 and row < (config.DIM) and col <  (config.DIM):
        ...
        ############ checks around current position if on the board #########################
            if row-1 >= 0 and col-1 >= 0 and row-1 < (config.DIM) and col-1 <  (config.DIM): #top left
                if config.board[row-1][col-1] == TentsConfig.TENT:
                    return tree
                else:
                    tree = True
                        
            elif row-1 >= 0 and col >= 0 and row-1 < (config.DIM) and col <  (config.DIM): #top mid
                if config.board[row-1][col] == TentsConfig.TENT:
                    return tree
                else:
                    tree = True
                        
            elif row-1 >= 0 and col+1 >= 0 and row-1 < (config.DIM) and col+1 <  (config.DIM): #top right
                if config.board[row-1][col+1] == TentsConfig.TENT:
                    return tree
                else:
                    tree = True
                        
            elif row >= 0 and col-1 >= 0 and row < (config.DIM) and col-1 <  (config.DIM): #mid left
                if config.board[row][col-1] == TentsConfig.TENT:
                    return tree
                else:
                    tree = True
                        
            elif row >= 0 and col+1 >= 0 and row < (config.DIM) and col+1 <  (config.DIM): #mid right
                if config.board[row][col+1] == TentsConfig.TENT:
                    return tree
                else:
                    tree = True

            elif row+1 >= 0 and col-1 >= 0 and row+1 < (config.DIM) and col-1 <  (config.DIM): #bottom left
                if config.board[row+1][col-1] == TentsConfig.TENT:
                    return tree
                else:
                    tree = True

            elif row+1 >= 0 and col >= 0 and row+1 < (config.DIM) and col <  (config.DIM): #bottom mid
                if config.board[row+1][col] == TentsConfig.TENT:
                    return tree
                else:
                    tree = True
                    
            elif row+1 >= 0 and col+1 >= 0 and row+1 < (config.DIM) and col+1 <  (config.DIM): #bottom right
                if config.board[row+1][col+1] == TentsConfig.TENT:
                    return tree
                else:
                    tree = True

            #################### if not a tent, then a tree... ########################################
            if row-1 >= 0 and col >= 0 and row-1 < (config.DIM) and col <  (config.DIM): #top mid
                if config.board[row-1][col] == TentsConfig.TREE:
                    tree = True
                else:
                    return tree

            if row+1 >= 0 and col >= 0 and row+1 < (config.DIM) and col <  (config.DIM): #bottom mid
                if config.board[row+1][col] == TentsConfig.TREE:
                    tree = True
                else:
                    return tree

            if row >= 0 and col+1 >= 0 and row < (config.DIM) and col+1 <  (config.DIM): #mid right
                if config.board[row][col+1] == TentsConfig.TREE:
                    tree = True
                else:
                    return tree

            if row >= 0 and col-1 >= 0 and row < (config.DIM) and col-1 <  (config.DIM): #mid left
                if config.board[row][col-1] == TentsConfig.TREE:
                    tree = True
                else:
                    return tree
