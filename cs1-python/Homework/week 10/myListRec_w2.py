...
def PyListToMylist(pylist):
    if pylist == []:
        print("pylist is empty")
    else:
        return pyListtoMyListRec(pylist)

def PyListToMyListRec(pylist):
    x=0
    if pylist[x] == []:
        return
    else:
        val=pylist[x]
        append(MyList.head,val)
        x+=1
        return pyListToMyListRec(val)
...
def remove(lst,value):
    ...
    if isinstance(lst,EmptyNode):
        return False
    if value == lst.head.data:
        lst.head=lst.head.next
        lst.size-=1
        return True
    else:
        val=removeRec(lst.head,value)
        if val == True:
            lst.head=lst.head.next
            lst.size-=1
            return True
