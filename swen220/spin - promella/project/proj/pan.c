...
{	int j, h = now._nr_pr;
	...
	case 2:	/* Server */
		...
		/* params: */
		/* locals: */
		((P2 *)pptr(h))->_4_thisOrder = 0;
#ifdef VAR_RANGES
		logval("Server:thisOrder", ((P2 *)pptr(h))->_4_thisOrder);
#endif
#ifdef HAS_CODE
		locinit2(h);
#endif
	...
	case 1:	/* Cashier */
		...
		((P1 *)pptr(h))->_3_cust = 0;
#ifdef VAR_RANGES
		logval("Cashier:cust", ((P1 *)pptr(h))->_3_cust);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* Customer */
		...
		/* locals: */
#ifdef VAR_RANGES
		logval("Customer:me", ((P0 *)pptr(h))->me);
		logval("Customer:food", ((P0 *)pptr(h))->food);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(void)
{
	...
	{	int l_in;
		...
		{
			now.waiting[l_in] = 255;
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			custsOrdering[l_in] = 255;
		}
	}
		now.nextWait = 0;
		now.nextOrder = 0;
	...
	{	int l_in;
		...
		logval("nextWait", now.nextWait);
		logval("nextOrder", now.nextOrder);
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			logval("order[l_in]", now.order[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			logval("orderId[l_in]", now.orderId[l_in]);
		}
	}
		logval("curServe", now.curServe);
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			logval("serve_foods[l_in]", now.serve_foods[l_in]);
		}
	}
	...
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			logval("cust_wait[l_in]", now.cust_wait[l_in]);
		}
	}
		logval("cash", now.cash);
		logval("serve", now.serve);
		logval("takeOrder", now.takeOrder);
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			logval("foods[l_in]", now.foods[l_in]);
		}
	}
#endif
...
{	/* int i; */
	printf("global vars:\n");
	printf("	byte   ordering:	%d\n", now.ordering);
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			printf("	byte   waiting[%d]:	%d\n", l_in, now.waiting[l_in]);
		}
	}
	printf("	byte   nextWait:	%d\n", now.nextWait);
	printf("	byte   nextOrder:	%d\n", now.nextOrder);
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			printf("	byte   order[%d]:	%d\n", l_in, now.order[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			printf("	byte   orderId[%d]:	%d\n", l_in, now.orderId[l_in]);
		}
	}
	printf("	byte   curServe:	%d\n", now.curServe);
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			printf("	byte   serve_foods[%d]:	%d\n", l_in, now.serve_foods[l_in]);
		}
	}
	printf("	byte   mutex:	%d\n", now.mutex);
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			printf("	byte   cust_wait[%d]:	%d\n", l_in, now.cust_wait[l_in]);
		}
	}
	printf("	byte   cash:	%d\n", now.cash);
	printf("	byte   serve:	%d\n", now.serve);
	printf("	byte   takeOrder:	%d\n", now.takeOrder);
}
void
...
{	/* int i; */
	...
	printf("	byte   thisOrder:	%d\n", ((P2 *)pptr(pid))->_4_thisOrder);
		break;
	case 1:
