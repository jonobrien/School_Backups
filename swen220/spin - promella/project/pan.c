...
/*** From source: shift-alt-escape_paste1_edits.pml ***/
...
{	int j, h = now._nr_pr;
...
#ifndef NOCLAIM
	case 7:	/* claim select */
		spin_c_typ[0] = 5; /* Liveness */
		((P7 *)pptr(h))->c_cur[0] = 3;
		reached5[3]=1;
		spin_c_typ[1] = 4; /* Safety */
		((P7 *)pptr(h))->c_cur[1] = 3;
		reached4[3]=1;
		((P7 *)pptr(h))->_t = 4;
		((P7 *)pptr(h))->_p = 3;
		((P7 *)pptr(h))->_n = 1; /* Safety */
		src_claim = src_ln4;
		if (whichclaim == -1 && claimname == NULL)
			printf("0: Claim Safety (4), from state 3\n");
		if (whichclaim != -1)
	...
	case 2:	/* Server */
		...
		/* params: */
		/* locals: */
		((P2 *)pptr(h))->_4_receivedOrder = 0;
		((P2 *)pptr(h))->_4_custID = 0;
#ifdef VAR_RANGES
		logval("Server:receivedOrder", ((P2 *)pptr(h))->_4_receivedOrder);
		logval("Server:custID", ((P2 *)pptr(h))->_4_custID);
#endif
#ifdef HAS_CODE
		locinit2(h);
#endif
	...
	case 1:	/* Cashier */
		...
		/* params: */
		/* locals: */
		((P1 *)pptr(h))->_3_someCustPlacedOrder = 0;
		((P1 *)pptr(h))->_3_custID = 0;
		((P1 *)pptr(h))->_3_custsOrder = 0;
#ifdef VAR_RANGES
		logval("Cashier:someCustPlacedOrder", ((P1 *)pptr(h))->_3_someCustPlacedOrder);
		logval("Cashier:custID", ((P1 *)pptr(h))->_3_custID);
		logval("Cashier:custsOrder", ((P1 *)pptr(h))->_3_custsOrder);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* Customer */
		...
		reached0[30]=1;
		/* params: */
		((P0 *)pptr(h))->favFood = par0;
		((P0 *)pptr(h))->id = par1;
		/* locals: */
#ifdef VAR_RANGES
		logval("Customer:favFood", ((P0 *)pptr(h))->favFood);
...
iniglobals(void)
{
	...
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			now.customerWaiting[l_in] = 0;
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			now.custPlacedOrder[l_in] = 0;
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			now.orderTaken[l_in] = 0;
		}
	}
	...
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			now.custOrderSent[l_in] = 0;
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 1; l_in++)
		{
			now.serverBusy[l_in] = 0;
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			now.orderFulfilled[l_in] = 0;
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			now.customerMutex[l_in] = 1;
		}
	}
		custMutex = 1;
		now.cashierMutex = 1;
	...
	{	int l_in;
		for (l_in = 0; l_in < 4; l_in++)
		{
			now.foodArray[l_in] = 0;
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			now.customerOrder[l_in] = 0;
		}
	}
#ifdef VAR_RANGES
		logval("cashierBusy", now.cashierBusy);
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			logval("customerWaiting[l_in]", now.customerWaiting[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			logval("custPlacedOrder[l_in]", now.custPlacedOrder[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			logval("orderTaken[l_in]", now.orderTaken[l_in]);
		}
	}
		logval("someCustSelected", now.someCustSelected);
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			logval("custOrderSent[l_in]", now.custOrderSent[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 1; l_in++)
		{
			logval("serverBusy[l_in]", now.serverBusy[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			logval("orderFulfilled[l_in]", now.orderFulfilled[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			logval("customerMutex[l_in]", now.customerMutex[l_in]);
		}
	}
		logval("cashierMutex", now.cashierMutex);
	{	int l_in;
		for (l_in = 0; l_in < 1; l_in++)
		{
			logval("serverMutex[l_in]", now.serverMutex[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 4; l_in++)
		{
			logval("foodArray[l_in]", now.foodArray[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			logval("customerOrder[l_in]", now.customerOrder[l_in]);
		}
	}
#endif
...
{	/* int i; */
	...
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			printf("	byte   customerMutex[%d]:	%d\n", l_in, now.customerMutex[l_in]);
		}
	}
	printf("	byte   cashierMutex:	%d\n", now.cashierMutex);
	{	int l_in;
		for (l_in = 0; l_in < 1; l_in++)
		{
			printf("	byte   serverMutex[%d]:	%d\n", l_in, now.serverMutex[l_in]);
		}
	}
	printf("	bit    cashierBusy:	%d\n", now.cashierBusy);
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			printf("	bit    customerWaiting[%d]:	%d\n", l_in, now.customerWaiting[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			printf("	bit    custPlacedOrder[%d]:	%d\n", l_in, now.custPlacedOrder[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			printf("	bit    orderTaken[%d]:	%d\n", l_in, now.orderTaken[l_in]);
		}
	}
	printf("	bit    someCustSelected:	%d\n", now.someCustSelected);
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			printf("	bit    custOrderSent[%d]:	%d\n", l_in, now.custOrderSent[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 1; l_in++)
		{
			printf("	bit    serverBusy[%d]:	%d\n", l_in, now.serverBusy[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 3; l_in++)
		{
			printf("	bit    orderFulfilled[%d]:	%d\n", l_in, now.orderFulfilled[l_in]);
		}
	}
}
void
...
{	/* int i; */
	...
	printf("	bit    receivedOrder:	%d\n", ((P2 *)pptr(pid))->_4_receivedOrder);
	printf("	byte   custID:	%d\n", ((P2 *)pptr(pid))->_4_custID);
		break;
	case 1:
		printf("local vars proc %d (Cashier):\n", pid);
	printf("	bit    someCustPlacedOrder:	%d\n", ((P1 *)pptr(pid))->_3_someCustPlacedOrder);
	printf("	byte   custID:	%d\n", ((P1 *)pptr(pid))->_3_custID);
		break;
	case 0:
