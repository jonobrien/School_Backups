//Jon O'Brien
//Enrollment Static to Dynamic Activity
...
pred enroll[p : Person, c : Course, st : Step] {
	//Preconditions - what needs to be true for the step to occur
	st != last
	p ! in c.enrolled.st
	c ! in p.transcript
	c.prereqs in p.transcript
	...
	let st' = next[st] {
		 //Effects - things affected by step
		c.enrolled.st' = c.enrolled.st + p
		 //Frame - keep everything else constant
