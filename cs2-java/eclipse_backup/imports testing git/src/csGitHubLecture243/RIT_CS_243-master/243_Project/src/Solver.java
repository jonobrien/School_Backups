...
public class Solver
{
	 ...
	 */
	public ArrayList<Integer> solve(Puzzle c)
	{
		ArrayList<ArrayList <Integer>> queue = new ArrayList<ArrayList <Integer>>();
		ArrayList<Integer> current = new ArrayList<Integer>();
		current.add(c.getStart());
		queue.add(current);
		boolean found = (c.getGoal() == c.getStart());

		while (queue.size() != 0 && !found)
		{
			current = queue.remove(0);
			for (int neighbor : c.getNeighbors(current.get(current.size()-1)))
			{
