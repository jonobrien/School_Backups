...
public class InsertHTMLConstructCommand implements CommandInterface, ActionListener {
    ...
    private void insertTag(boolean moveCaretBack, HTMLTag tag){
        ...
        int oldIndent = 0;
        int oPosition = buffer.getCaretPosition();
        String textInserted = "";
        try {
        ...
        textInserted = textInserted.concat( tag.getOpenTag() );
        buffer.insert( tag.getOpenTag() , buffer.getCaretPosition() );
        //If it's not inline start adding newlines with correct indentation
        if(!tag.getInLine()){
            textInserted = textInserted.concat( "\n" + StaticUtilities.spaces(oldIndent + StaticUtilities.getTabSpaces()) );
        ...
        //If it's not inline start adding newlines with correct indentation
        if(!tag.getInLine()){
            textInserted = textInserted.concat( "\n" + StaticUtilities.spaces(oldIndent) );
            buffer.insert( "\n" + StaticUtilities.spaces(oldIndent), buffer.getCaretPosition() );
        }
        //Insert closing tag
        textInserted = textInserted.concat( tag.getCloseTag() );
        buffer.insert( tag.getCloseTag() , buffer.getCaretPosition() );
        //Move back to the middle of the tag if required
        StaticUtilities.getCurrentTab().getUndoBuffer().addInsertAction( textInserted , oPosition );
    ...
    private void insertNewLine(){
        ...
        StaticUtilities.getCurrentTab().getUndoBuffer().addInsertAction( "\n" + StaticUtilities.spaces(oldIndent) , buffer.getCaretPosition() );
        buffer.insert( "\n" + StaticUtilities.spaces(oldIndent), buffer.getCaretPosition() );
    }
    ...
    private void insertURL(boolean anchor){
        ...
        //Add the new line and the found indentation
        StaticUtilities.getCurrentTab().getUndoBuffer().addInsertAction( URL , buffer.getCaretPosition() - (anchor ? 2:0));
